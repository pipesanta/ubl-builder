...
const ParamsMap: IGenericKeyValue<ParamsMapValues> = {
    ...
    jurisdictionRegionAddress: { order: 5,  attributeName: 'cac:JurisdictionRegionAddress', min: 0, max: 1, classRef: JurisdictionRegionAddress }
}
...
class TaxScheme extends GenericAggregateComponent {
   ...
   * @returns { UdtIdentifier | String } Tax's id
   */
  getId(rawValue= true){
  ...
  setName(value: string | UdtName){
    this.attributes.name = (value instanceof UdtName)
      ? value
      : new UdtName(value)
  }
...
export  {
   TaxScheme,
   AllowedParams as TaxSchemeParams,   
}
