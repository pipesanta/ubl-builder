...
const ParamsMap: IGenericKeyValue<ParamsMapValues> = {
  ...
  orderableUnitFactorRate: { order: 6,  attributeName: 'cbc:OrderableUnitFactorRate', min: 0, max:1, classRef: UdtRate },
  ...
  allowanceCharges: { order: 9,  attributeName: 'cac:AllowanceCharge', min: 0, max: undefined, classRef: AllowanceCharge },
  pricingExchangeRate: { order: 9,  attributeName: 'cac:PricingExchangeRate', min: 0, max: 1, classRef: PricingExchangeRate },
...
class Price extends GenericAggregateComponent {
  ...
  setPriceAmount(value: string | UdtAmount){
    this.attributes.priceAmount = ( value instanceof UdtAmount )
      ? value
      : new UdtAmount(value);
  }
  ...
  setBaseQuantity(value: string | UdtQuantity ){
    this.attributes.baseQuantity = ( value instanceof UdtQuantity )
      ? value
      : new UdtQuantity(value);
  }
