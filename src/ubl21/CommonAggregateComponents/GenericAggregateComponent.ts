...
export default class GenericAggregateComponent {
    ...
    Object.keys(this.paramsMap)
      ...
      .forEach(attKey => {
          ...
          if( Array.isArray(this.attributes[attKey]) && max !== undefined ){
            throw "array given and max is defined validate structure"
          }
      ...
      .forEach(att => {
        ...
        if (Array.isArray(content[att])) {
          this.attributes[att] = content[att].map((subItem: any) =>
            subItem instanceof AbstractClass
              ? subItem
              : new AbstractClass(subItem.content, subItem.attributes)
          );
        } else {
          ...
          this.attributes[att] = (content[att] instanceof AbstractClass)
              ? content[att]
              : new AbstractClass(childContent, childAttributes);
        }
      });
  }
   ...
   * 
   */
  printAsXml(pretty=true, headless=false){
   ...
   */
  printAsJson(deep = false){
    const json = this.parseToJson();
    if(deep){ console.log(JSON.stringify(json))}
