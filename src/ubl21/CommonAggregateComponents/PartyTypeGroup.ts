...
const ParamsMap: IGenericKeyValue<ParamsMapValues> = {
    ...
    markAttentionIndicator: { order: 2,  attributeName: 'cbc:MarkAttentionIndicator', min: 0, max:1, classRef: UdtIndicator },
    ...
    industryClassificationCode: { order: 6,  attributeName: 'cbc:IndustryClassificationCode', min: 0, max: 1, classRef: UdtCode },
    partyIdentifications: { order: 7,  attributeName: 'cac:PartyIdentification', min: 0, max: undefined, classRef: PartyIdentification },
    ...
    partyLegalEntities: { order: 13,  attributeName: 'cac:PartyLegalEntity', min: 0, max: undefined, classRef: PartyLegalEntity }, 
...
class Party extends GenericAggregateComponent {
  ...
  addPartyName(value: PartyName | string){
    ...
    if( !(value instanceof PartyName) && typeof value !== "string" ){
      throw "Value must be instance of PartyName or a String"
    }
    const itemToPush = ( value instanceof PartyName )
      ? value
      : new PartyName({ name: value });
    this.attributes.partyNames.push(itemToPush);
  }

  setPhysicalLocation(value: PhysicalLocation ){

    if(!(value instanceof PhysicalLocation)) throw "Value must be instance of PhysicalLocation "
  ...
  addPartyTaxScheme(value: PartyTaxScheme | PartyTaxSchemeParams){
    ...
    const itemToPush = (value instanceof PartyTaxScheme)
      ? value
      : new PartyTaxScheme(value);
  ...
  addPartyLegalEntity(value: PartyLegalEntity){
    ...
    if(value instanceof PartyLegalEntity){
      ...
      throw "Value must to be instance of PartyLegalEntity"
    }
  }
  ...
  setContact(value: Contact){
    if(!(value instanceof Contact)) throw "Value must to be instance of Contact";
    this.attributes.contact = value;
  }


  addPartyIdentification(value: PartyIdentification | PartyIdentificationParams ){
    if(!this.attributes.partyIdentifications) { this.attributes.partyIdentifications = []; }
    const itemToPush = ( value instanceof PartyIdentification )
      ? value
      : new PartyIdentification(value);
