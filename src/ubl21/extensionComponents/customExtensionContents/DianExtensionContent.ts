...
const ParamsMap: IGenericKeyValue<ParamsMapValues> = {
  ...
  softwareSecurityCode: { order: 4,  attributeName: 'sts:SoftwareSecurityCode', min: 0, max: 1, classRef: UdtIdentifier },
  authorizationProvider: { order: 5,  attributeName: 'sts:AuthorizationProvider', min: 0, max: 1, classRef: AuthorizationProvider },
...
class DianExtensionsContent extends GenericAggregateComponent {
  ...
  setQRCode(value: string | UdtText){
    this.attributes.QRCode = ( value instanceof UdtText )
      ? value
      : new UdtText(value)
  }
  ...
  getQRCode(raw = true){
    return raw 
      ? (this.attributes.QRCode || {}).content 
...
export {
  DianExtensionsContent as DianExtensionsContent,
  AllowedParams as DianExtensionsContentParams,   
}
